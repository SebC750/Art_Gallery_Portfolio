...
function merge(list, left, split, right, compareType){
    ...
    let sort_left_idx = 0, sort_right_idx = 0
    let merge_idx = left
...
function mergeSort(list: Artwork[],left: number,right:number ,compareType){
...
export function sortByTitle(list: Artwork[]){
  try{
...
export function sortByDate(list: Artwork[]){
...
function compareByTitle(titleA:any,titleB:any){
  return titleA.title.localeCompare(titleB.title)
}
function compareByDate(dateA:any, dateB:any){
